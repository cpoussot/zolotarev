...
Bnum    = (z.^(deg_num:-1:0)).';
Bden    = (z.^(deg_den:-1:0)).';
name    = {['Opt., $\sigma_{' num2str(robj) '}=' num2str(sig_opt,2) '$'] ...
           ['LF, $\sigma_{' num2str(robj)  '}=' num2str(SIG(1),2) '$'] ...
           ['AAA, $\sigma_{' num2str(robj) '} =' num2str(SIG(2),2) '$'] ...
           ['AAA-Lawson, $\sigma_{' num2str(robj) '}=' num2str(SIG(3),2) '$']};
str = latex_tab(Bnum,Bden,n4_opt,d4_opt,NUM,DEN,name);
str
%%
%%% Plot
g4_opt  = matlabFunction(sum(n4_opt.*Bnum)/sum(d4_opt.*Bden));
g4_loe  = matlabFunction(sum(NUM(:,1).*Bnum)/sum(DEN(:,1).*Bden));
g4_loeI = matlabFunction(sum(DEN(:,1).*Bden)/sum(NUM(:,1).*Bnum));
...
switch CAS
    ...
    case '1b'
        ...
        %plot(xx,abs(g4_loeI_-g4_opt_),'-','Color','m')
...
if SAVEIT; zol.figSavePDF(['figures/specials/case' CAS '_r' num2str(robj)]); end;
