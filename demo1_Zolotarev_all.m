...
for idxCas = 1:numel(spaceCAS)
    ...
    %[h4,info]       = zol.loewner_simple(val,pts,robj0,n1,n2,d);
    ...
    Zr_loe          = zeros(numel(yy),numel(xx));
    for i = 1:numel(xx)
        for j = 1:numel(yy)
    ...
    %%%
    figure, 
    %subplot(ceil(K/2),2,1,'Color',[229 255 204]/255), hold on, grid on
    subplot(ceil(K/2),2,1), hold on, grid on
    ...
    title({'\textbf{LF}'; ['$r=$' num2str(robj) ', $\sigma_r=$ ' num2str(abs(hsig)) ' in ' num2str(timeLOE) 's' ]}, 'Interpreter','latex')
    ...
    for ii = 1:numel(AAAparam)
        ...
        %
        subplot(ceil(K/2),2,kk), hold on, grid on
    ...
    lf.figSavePDF(['figures/all/cas_' CAS '_r' num2str(robj0) ],.6)
end
